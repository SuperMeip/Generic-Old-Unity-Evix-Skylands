...
namespace MeepTech.Voxel.Generation.Mesh {
  ...
  /// </summary>
  public class MarchGenerator : IVoxelMeshGenerator {
    ...
    /// <returns></returns>
    public IMesh generateMesh(IVoxelStorage blockData) {
      return MarchOverBlocks(blockData);
    }
    ...
    /// <returns></returns>
    static IMesh MarchOverBlocks(IVoxelStorage blockData) {
      IVoxelStorage points = blockData;
      int vertexIndex = 0;
      Vector3[] blockVertexes = new Vector3[12];
      Color[] blockVertexUVValues = new Color[12];
      IMesh mesh = new Mesh();

      // March over each point
      Coordinate.Zero.until(points.bounds, (coordinate) => {
        March(coordinate, blockData, ref vertexIndex, blockVertexes, blockVertexUVValues, mesh);
      });
    ...
    /// <summary>
    /// March over the points for a single block.
    /// </summary>
    /// <param name="pointLocation"></param>
    static void March(Coordinate pointLocation, IVoxelStorage allPoints, ref int vertexIndex, Vector3[] blockVertexes, Color[] blockVertexUVValues, IMesh mesh) {
      int blockMask = 0;
      ...
      foreach (Octants.Octant octant in Octants.All) {
        ...
        blockVertexLocations[octant.Value] = blockVertexLocation.vec3 * World.BlockSize;
        blockTypes[octant.Value] = allPoints.get(blockVertexLocation);
      ...
      for (int currentTriangleStartPointIndex = 0; currentTriangleStartPointIndex < rowOfTriangles.Length; currentTriangleStartPointIndex += 3) {
        ...
        mesh.addVertexWithColor(blockVertexes[rowOfTriangles[currentTriangleStartPointIndex + 0]], blockVertexUVValues[rowOfTriangles[currentTriangleStartPointIndex + 0]]);
        mesh.triangles.Add(vertexIndex);
        vertexIndex++;

        mesh.addVertexWithColor(blockVertexes[rowOfTriangles[currentTriangleStartPointIndex + 1]], blockVertexUVValues[rowOfTriangles[currentTriangleStartPointIndex + 1]]);
        mesh.triangles.Add(vertexIndex);
        vertexIndex++;

        mesh.addVertexWithColor(blockVertexes[rowOfTriangles[currentTriangleStartPointIndex + 2]], blockVertexUVValues[rowOfTriangles[currentTriangleStartPointIndex + 2]]);
        mesh.triangles.Add(vertexIndex);
        vertexIndex++;
      }
    }
